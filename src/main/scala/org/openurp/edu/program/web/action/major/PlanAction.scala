...
class PlanAction extends ActionSupport, EntityAction[MajorPlan], ProjectSupport {
  ...
  def groups(): View = {
    ...
    val majorPlan = entityDao.findBy(classOf[MajorPlan], "program", program).headOption match
      case None =>
  ...
  def saveGroup(): View = {
    ...
    if (getBoolean("toGroups", false)) {
      redirect("groups", "program.id=" + plan.program.id, "info.save.success")
    } else {
      redirect("edit", "program.id=" + plan.program.id, "info.save.success")
    }
  }
  ...
  def groupCourses(): View = {
    ...
    //    put("weekstateDigestor", new NumberRangeDigestor)
    forward()
  }
  ...
  def restat(): View = {
    val plan = entityDao.get(classOf[MajorPlan], getLongId("plan"))
    planService.statPlanCredits(plan)
    redirect("edit", s"program.id=${plan.program.id}", "统计成功")
  }
  ...
  def saveCourse(): View = {
    ...
    var extra = "&courseGroup.id=" + group.id + "&planId=" + plan.id
    val target =
      if getBoolean("toGroups", false) then "groupCourses"
      else
        extra = "program.id=" + plan.program.id;
        "edit"
  ...
  def removeCourse(): View = {
    ...
    if getBoolean("toGroups", false) then
      redirect("groupCourses", "&courseGroup.id=" + group.id, "info.save.success")
    else
      redirect("edit", "program.id=" + group.plan.program.id, "info.remove.success")
  }
